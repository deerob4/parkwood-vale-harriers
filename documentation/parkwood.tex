...
\hline
\textbf{Field Name}     & \textbf{Primary Key} & \textbf{Typical Data} & \textbf{Data Type} & \textbf{Data Type} \\ \hline
...
This process is used to modify the personal info of the user. It checks which element the user wants to change, and then matches their changed element against a regular expression to ensure they are valid. 
...
This function will calculate the number of calories burned in the training session. It builds on the idea suggested in the brief by taking into account how well a user thought the session went, and also taking into account the weight of the user.
...
The system must also be able to add a training session in at least 2 seconds from when the user clicked the add button, and the database should be able to store at least 100 users.

\subsection{Non-Measurable Criterion}
The system should be very easy to use. It should have sensible, coherent navigation to allow the user to easily move between each section of the system. 

Additionally, the system should have a consistent layout on each of the pages; this will serve to ensure that the user does not get lost when navigating through the system: the navigation bar, logo and footer should always be in the same location.

It is also important that the buttons and other controls, such as inputs, on the form are easy for the user to understand - they should know exactly what each button does, and what the program will do when they click it. 
...
\subsection{Profile Page}
The profile page allows user to view and update their personal information. Certain sections appear on demand, so multiple captures have been taken.
...
The interface for changing details is very simple - it features just an input for changing element, and a button to confirm. The placeholder text for the input is set to current item, for visual consistency. Making this panel pop up as opposed to being on a seperate page improves flow of page, preventing user becoming disorientated.
...
As can be seen, all of the validation tests passed - they all result in appropriate error messages being shown to the user, as is written in the processes. Additionally, the system correctly identifies if a user enters an incorrect email or password, an important security feature.
...
This section tests the system's ability to delete the user. Log in as Katrina Smith:
\begin{description}[labelindent=1cm]
  \item[Email address:] katrina@gmail.com
  \item[Password:] pythonisdifficult
\end{description}
...
You should be redirected to the login page with a message saying ``Your account has been delete. Sorry to see you go!'' and the user should be removed from the Users Table. All their activities should be removed.
...
As can be seen, the user was deleted perfectly; the test succeeded.
...
Create a user with a weight of 87kg. Add a running session with the following parameters:
