...
target.click()}})}
return P.trigger('open')}, close:function(giveFocus){if(giveFocus){
...
function prepareElementRoot(){P.$root.on({keydown:handleKeydownEvent,
focusin:function(event){P.$root.removeClass(CLASSES.focused)
event.stopPropagation()},
'mousedown click':function(event){var target=event.target
...
return PickerConstructor}));$(document).ready(function(){var $startInput=$('.input-swim-start').pickatime();var $startPicker=$startInput.pickatime('picker');var $finishInput=$('.input-swim-finish').pickatime();var $finishPicker=$finishInput.pickatime();$('.activity-picker .glyphicon').click(function(){var $this=$(this);$this.parent('.activity-block').animate({height:'-=445'},350);});$('.activity-picker li').not('.animated').click(function(){expandActivity($(this))});function expandActivity($activity){$activity.animate({height:'520'},{duration:350,queue:false});$activity.find('input').css('display','block').addClass('animated fadeInUp')}});